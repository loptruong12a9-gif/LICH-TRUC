document.addEventListener('DOMContentLoaded', () => {
     ...
     */
    async function getBase64Image(imgUrl, timeoutMs = 10000) {
        return new Promise((resolve) => {
            const img = new Image();
            img.crossOrigin = 'Anonymous';
            const timer = setTimeout(() => {
                img.src = '';
                console.error("? getBase64Image timed out for:", imgUrl);
                resolve(null);
            }, timeoutMs);
    ...
    /**
     * Helper to validate logo data
     */
    function isValidBase64(str) {
        return typeof str === 'string' && str.length > 500 && str.startsWith('data:image');
    }

    /**
     * loadLogoFromUrl: Fetches an image URL and returns Base64.
     * Robust against CORS if local; handles errors gracefully.
     */
    ...
    // Ensures logo is ready before export - HIGH PRIORITY FIX
    async function repairLogo() {
        console.log("Repairing logo...");

        // 1. PRIORITY: Try to load fresh from file with retry (Best Quality)
        for (let attempt = 1; attempt <= 3; attempt++) {
            console.log(`Attempt ${attempt}/3 to load logo from file...`);
            const freshLogo = await loadLogoFromUrl('./logo.jpg');
            if (freshLogo && isValidBase64(freshLogo)) {
                console.log("✓ Logo loaded successfully from file!");
                state.logoBase64 = freshLogo;
                return freshLogo;
            }
            // Wait before retry
        ...
        // 2. Fallback to existing state if valid
        if (state.logoBase64 && isValidBase64(state.logoBase64)) {
            console.log("✓ Using cached logo state.");
            return state.logoBase64;
        }

        // 3. Ultimate Fallback: Embedded Base64 (May be corrupted - last resort)
        if (!LOGO_BASE64_EMBEDDED || LOGO_BASE64_EMBEDDED.length < 100) {
            console.warn("⚠ WARNING: No valid embedded logo available.");
            return null;
        }
        console.log("ℹ Using embedded logo fallback (High Quality).");
        return LOGO_BASE64_EMBEDDED;
    }

    // Initial load (optional, repairLogo handles it on export)
    loadLogoFromUrl('./logo.jpg').then(b64 => {
        if (b64) state.logoBase64 = b64;
    });
    ...
    function bindEvents() {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            // Save Button
            if (target.id === 'saveStaffBtn' || target.closest('#saveStaffBtn')) {
                saveSettings();
    ...
    const exportFile = async (type) => { // Must be async for image
        ...
        // 1. Prepare Logo - ROBUST STRATEGY
        // Repair logo state if needed (async)
        const logoToUse = await repairLogo();

        // Use 'pt' units which are more native to Word (140px approx 105pt)
        // Use 'pt' units which are more native to Word (140px approx 105pt)
        let logoImgTag = logoToUse ? `<img src="${logoToUse}" width="105" height="105" style="width:105pt; height:105pt;">` : "";
