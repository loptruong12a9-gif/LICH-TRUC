document.addEventListener('DOMContentLoaded', () => {
    ...
    function getHolidayName(date) {
        ...
        // --- 1. Fixed Solar Holidays (Dương Lịch) ---
        ...
        // --- 2. Dynamic Lunar Holidays (Âm Lịch) ---
        // result = [day, month, year, leap]
        const lunarDate = convertSolar2Lunar(d, m, y, 7);
        const lDay = lunarDate[0];
        const lMonth = lunarDate[1];
        const lLeap = lunarDate[3];

        const lunarKey = `${lDay}/${lMonth}`;

        // Only check holidays on non-leap months (usually)
        if (lLeap === 0) {
            if (lunarKey === '1/1') return 'Mùng 1 Tết';
            if (lunarKey === '2/1') return 'Mùng 2 Tết';
            if (lunarKey === '3/1') return 'Mùng 3 Tết';
            if (lunarKey === '10/3') return 'Giỗ Tổ Hùng Vương';
        }
    ...
    function SunLongitude(jdn) {
        ...
        let T2 = T * T;
        let dr = PI / 180;
        let M = 357.52910 + 35999.05030 * T - 0.0001559 * T2 - 0.00000048 * T * T * T;
    ...
    const exportFile = async (type) => { // Must be async for image
        ...
        // 1. Prepare Logo - EXTREME FIX (Hardcoded Base64)
        // We use the embedded constant directly, bypassing state
        const finalLogoBase64 = LOGO_BASE64_EMBEDDED;

        // Use 'pt' units which are more native to Word (140px approx 105pt)
        let logoImgTag = `<img src="${finalLogoBase64}" width="105" height="105" style="width:105pt; height:105pt;">`;

        // Use a span for fallback text, not div, to avoid block-breaking in table cells
        logoImgTag += `<br><span style="font-size:8pt; color:#888;">(Logo BV)</span>`;
